...
describe("annot-doc-shell: step block URL chip", () => {
  function makeStepDocWithLink(): AnnotDocument {
    return {
      version: 1,
      lang: "en",
      title: "Linked step doc",
      meta: { title: "Linked step doc" },
      styleBlock: null,
      blocks: [
        {
          kind: "step",
  ...
  it("renders the chip in read-only mode with the right href and label", async () => {
    const el = mount(makeStepDocWithLink());
    el.editing = false;
    await el.updateComplete;
    const chip = el.querySelector("[data-step-link]") as HTMLAnchorElement | null;
    expect(chip).not.toBeNull();
    expect(chip?.getAttribute("href")).toBe("https://example.com");
    expect(chip?.getAttribute("target")).toBe("_blank");
    expect(chip?.getAttribute("rel")).toBe("noopener noreferrer");
    expect(chip?.textContent?.trim()).toBe("Docs");
  });

  it("renders the URL input row in editing mode pre-filled with the current URL", async () => {
    const el = mount(makeStepDocWithLink());
    el.editing = true;
    await el.updateComplete;
    const input = el.querySelector("[data-step-link-input]") as HTMLInputElement | null;
    expect(input).not.toBeNull();
    expect(input?.value).toBe("https://example.com");
  });

  it("does not render the chip or editor when block.link is absent", async () => {
    const doc = makeStepDocWithLink();
    const blockWithoutLink = { ...doc.blocks[0], link: undefined } as Block;
    const docWithout: AnnotDocument = { ...doc, blocks: [blockWithoutLink] };
    const el = mount(docWithout);
    el.editing = false;
    await el.updateComplete;
    expect(el.querySelector("[data-step-link]")).toBeNull();
  });

  it("commits a valid URL on change and updates block.link", async () => {
    const noLinkDoc: AnnotDocument = {
      version: 1,
      lang: "en",
      title: "T",
      meta: { title: "T" },
    ...
    const el = mount(noLinkDoc);
    el.editing = true;
    await el.updateComplete;
    const input = el.querySelector("[data-step-link-input]") as HTMLInputElement;
    input.value = "https://new.example.com";
    input.dispatchEvent(new Event("change", { bubbles: true }));
    await el.updateComplete;
    const step = el.document!.blocks[0];
    if (step?.kind !== "step") throw new Error("expected step");
    expect(step.link).toEqual({ url: "https://new.example.com" });
  });

  it("clears block.link when the input is emptied", async () => {
    const el = mount(makeStepDocWithLink());
    el.editing = true;
    await el.updateComplete;
    const input = el.querySelector("[data-step-link-input]") as HTMLInputElement;
  ...
  it("drops a javascript: URL silently (treats as empty)", async () => {
    const noLinkDoc: AnnotDocument = {
      version: 1,
      lang: "en",
      title: "T",
  ...
  it("clicks on the chip do NOT open the image modal", async () => {
    const el = mount(makeStepDocWithLink());
    el.editing = true;
    await el.updateComplete;
    const openSpy = vi
      ...
      .mockResolvedValue({ kind: "cancel" });
    const chip = el.querySelector("[data-step-link]") as HTMLAnchorElement;
    // Suppress default navigation in the test environment so we
    // can observe whether the modal-opening handler fires.
