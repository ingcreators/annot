/**
 * Stories for `FileDetailsDrawer` — the full drawer host with
 * its chrome (backdrop + header + close button) plus every
 * visible combination of built-in sections (File / Tags /
 * Last commit / External links).
 *
 * Phase 1 initial landmark of `docs/plans/storybook-introduction.md`.
 * Each story opens the drawer automatically so reviewers see
 * the populated panel without having to click "toggle".
 */

import type { Meta, StoryObj } from "@storybook/web-components-vite";
import { FileDetailsDrawer, type FileDetailsData } from "./file-details-drawer.js";
...
function mountDrawer(args: Args): HTMLElement {
  ...
  const drawer = new FileDetailsDrawer(container, args.data, {
    isBuiltinSectionDisabled: (id) => args.disabledSections.includes(id),
  });
  drawer.open();
  return container;
}
...
const meta: Meta<Args> = {
  ...
  argTypes: {
    data: { control: false },
    disabledSections: { control: "check", options: [
      "drawer.file",
      "drawer.tags",
      "drawer.last-commit",
