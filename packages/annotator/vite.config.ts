...
export default defineConfig({
  ...
  build: {
    ...
    rollupOptions: {
      ...
      // `@napi-rs/canvas` (native binding) MUST stay external —
      // consumers install the platform-matched binary themselves
      // via npm. `pako` is a runtime dep of the inlined annot-core
      // PNG-8 encoder; keep it external so we don't ship a
      // duplicate copy in the bundle.
      external: ["@resvg/resvg-js", "@xmldom/xmldom", "@napi-rs/canvas", "pako"],
    },
  },
});
