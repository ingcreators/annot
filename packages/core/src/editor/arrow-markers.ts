...
// ─── Per-end arrow shape / size preview SVGs ────────────────────────
// Pure SVG-string builders for the per-end arrow Type / Size pulldown
// option previews. Moved from `@ingcreators/annot-editor/property-
// panel-helpers` (Tier C) into Tier B by Phase C of
// `docs/plans/property-panel-schema-extensions.md` so the registry's
// `arrowStartShape` / `arrowEndShape` defs can populate `iconSvg`
// option fields without a Tier B → Tier C reverse import. The
// `ArrowDim` / `ArrowShape` types are already imported at the top of
// this file via `tool-options.js` — these helpers reuse those.
...
export function arrowPreviewContent(shape: ArrowShape): string {
  ...
  switch (shape) {
    ...
    case "arrow":
      // Open chevron with thick rounded outline — matches PowerPoint's
      // "arrow" preset. Stem extends through to the tip so the stem
      // line has no visible gap at the opening.
