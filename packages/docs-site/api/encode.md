...
The encode pipeline is **separate from the [annotation DSL](./dsl.md)**.
...
| Value | Behaviour |
| --- | --- |
| `"smart"` *(default)* | Sample unique-colour count. If photo-heavy, emit `smartFallback`. Otherwise quantize to ≤256 colours via libimagequant and emit **PNG-8**. |
...
## License note: PNG-8 requires libimagequant (GPL-3.0)

`@ingcreators/annot-annotator` declares
`@ingcreators/annot-imagequant` as a regular `dependencies`
entry; it's auto-installed alongside the annotator. The package
is **GPL-3.0** (inherits libimagequant's licence).

Consumers who can't take on the GPL inclusion can
`npm uninstall @ingcreators/annot-imagequant` after install.
Smart mode then falls back gracefully to PNG-32 with
`reason: "imagequant-missing"`. The annotator's own code stays
Apache-2.0.
