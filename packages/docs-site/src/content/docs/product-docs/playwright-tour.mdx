...
description: The tour file shape, the productDocs fixture, sync conventions, scaling to dozens of screens.
---
...
Same shape: one `productDocs.sync(...)` per screen, in narrative
...
Two `productDocs.sync` calls in the tour, both passing the same
...
`productDocs.sync` doesn't throw when an `<Overlay match>` fails
