...
description: annot-mcp ships a stdio MCP server. The bin is exposed via npm so MCP clients can spawn it with npx — wire into Claude Desktop / Claude Code / Cursor.
---
...
(`annot_annotate_url`, `annot_redact_url`, `annot_aria_snapshot`).
The `_screenshot` tools that take pre-captured PNG bytes work
without it.
...
Restart Claude Desktop. The `annot_*` tools appear alongside
...
Restart Claude Code; the tools are available in chat.
...
{"result":{"protocolVersion":"2024-11-05",
           "capabilities":{"tools":{}},
           "serverInfo":{"name":"annot-mcp","version":"0.3.3"}},
 "jsonrpc":"2.0","id":1}
```
