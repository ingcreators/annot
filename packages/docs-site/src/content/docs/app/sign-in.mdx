---
title: Sign in
description: The web app works fully offline — sign-in is required only when you want cloud sync. GitHub + Google flows.
---

The web app works fully offline — you can use it without signing in.
Sign-in is required only when you want cloud sync.

## GitHub sign-in

Click **Sign in with GitHub** in the top-right of the web app.

1. The web app opens a popup at `annot.work/api/auth/github/start`.
2. GitHub asks you to authorise the `Annot` OAuth app.
3. On approval, the popup closes and the web app reloads with your
   account active.
...
separate; the web app asks for the `drive.file` scope only when
you explicitly switch to that backend.
