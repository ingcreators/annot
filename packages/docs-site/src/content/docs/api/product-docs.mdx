...
description: API reference for the living-product-docs core — parseMdx, the Playwright screen fixture, detectDrift, the annot-docs CLI.
---
...
test("login", async ({ page, screen }) => {
  ...
  await screen.capture({ id: "login", mdxPath: "docs/spec/SC-001.mdx" });
});
```

`screen.capture(options)` re-syncs the `annot:snapshot` +
