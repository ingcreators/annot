...
Numbered legend marker for "this is item N in a step-by-step
overlay over a screenshot". Composes a target rect outline + a
filled intent-coloured circle at one corner + a bold white
number inside the circle. The badge sits ON the target (not next
to it) so the legend stays readable when the screenshot is
scaled down in docs / slides.
...
When `placement` is `"auto"` (the default) and both
