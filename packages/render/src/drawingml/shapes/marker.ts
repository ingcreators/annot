...
export function buildMarker(s: AnnotationShape, id: number, ns: NamespaceOpts): string {
  ...
  // shrinks text to fit.
  return `<${ns.shape}><${ns.nvShape}><${ns.cnvPr} id="${id}" name="M${id}"/><${ns.cnvSp}/>${ns.nvPrSuffix}</${ns.nvShape}><${ns.spPr}><a:xfrm><a:off x="${px(cx - r)}" y="${px(cy - r)}"/><a:ext cx="${px(r * 2)}" cy="${px(r * 2)}"/></a:xfrm>${geom}<a:solidFill><a:srgbClr val="${fill}"/></a:solidFill><a:ln w="14288"><a:solidFill><a:srgbClr val="FFFFFF"/></a:solidFill></a:ln></${ns.spPr}>${ns.txBodyOpen}<a:bodyPr anchor="ctr" lIns="0" tIns="0" rIns="0" bIns="0" wrap="none"><a:normAutofit/></a:bodyPr><a:lstStyle/><a:p><a:pPr algn="ctr"/><a:r><a:rPr lang="en-US" sz="${ptVal}" b="1"><a:solidFill><a:srgbClr val="FFFFFF"/></a:solidFill></a:rPr><a:t>${exml(label)}</a:t></a:r></a:p>${ns.txBodyClose}</${ns.shape}>`;
}
