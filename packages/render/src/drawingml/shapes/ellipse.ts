...
export function buildEllipse(s: AnnotationShape, id: number, ns: NamespaceOpts): string {
  ...
  return `<${ns.shape}><${ns.nvShape}><${ns.cnvPr} id="${id}" name="E${id}"/><${ns.cnvSp}/></${ns.nvShape}><a:spPr><a:xfrm${xf}><a:off x="${px(cx - rx)}" y="${px(cy - ry)}"/><a:ext cx="${px(rx * 2)}" cy="${px(ry * 2)}"/></a:xfrm><a:prstGeom prst="ellipse"><a:avLst/></a:prstGeom>${fillXml}<a:ln w="${sw}"${cap}>${strokePaintXml(s, stroke)}${join}${dash}</a:ln></a:spPr></${ns.shape}>`;
}
