...
export function buildFreehand(s: AnnotationShape, id: number, ns: NamespaceOpts): string {
  ...
  return `<${ns.shape}><${ns.nvShape}><${ns.cnvPr} id="${id}" name="F${id}"/><${ns.cnvSp}/>${ns.nvPrSuffix}</${ns.nvShape}><a:spPr><a:xfrm${xf}><a:off x="${px(minX)}" y="${px(minY)}"/><a:ext cx="${pw}" cy="${ph}"/></a:xfrm><a:custGeom><a:avLst/><a:gdLst/><a:ahLst/><a:cxnLst/><a:rect l="0" t="0" r="${pw}" b="${ph}"/><a:pathLst><a:path w="${pw}" h="${ph}">${pathCmds}</a:path></a:pathLst></a:custGeom><a:noFill/><a:ln w="${sw}" cap="rnd"><a:solidFill><a:srgbClr val="${stroke}"/></a:solidFill>${dash}<a:round/></a:ln></a:spPr></${ns.shape}>`;
}
