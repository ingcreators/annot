...
## License note for embedding in commercial manuals

The PNG-8 path uses `@ingcreators/annot-imagequant` (GPL-3.0,
inherits libimagequant). If you're distributing the resulting
manuals as part of a commercial product and want to avoid the
GPL inclusion: `npm uninstall @ingcreators/annot-imagequant`.
Smart mode then falls back to PNG-32 (`reason:
"imagequant-missing"`). You lose the PNG-8 path but keep JPEG
fallback for photo-heavy content; the rest of the pipeline is
Apache-2.0.

## See also

- [Encode pipeline reference](../api/encode) — full options +
  defaults + tradeoffs.
- [Agent bug-report autopilot](./agent-bug-report) — same MCP
  composition pattern for a different goal.
- [`annot_annotate_url` schema](../ai-agents/tools#annot_annotate_url).
