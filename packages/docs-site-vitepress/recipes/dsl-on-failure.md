...
`annotateScreenshot()` accepts the [annotation DSL](../api/dsl)
...
[the SVG helpers page](../api/svg-helpers) still works and is
appropriate when:
...
[draw-by-DOM-locator recipe](./dom-locator) lifts it into a
helper.

For agent-side workflows where the agent has the locator string
but no `Page`, the
[MCP `annot_annotate_url` tool](../ai-agents/tools#annot_annotate_url)
