// `/api/usage` endpoint — Phase 4e.
//
// Returns the workspace's current usage + plan limits. The PWA
// uses this to render storage / document-count bars in the
// settings drawer + to surface the "approaching limit" banner
...
export async function handleUsageGet(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  // endpoint pays a 2-query D1 cost regardless.
  const [planRow, storageBytes, documentCount] = await Promise.all([
