...
// Moved verbatim from
// `packages/product-docs-astro/src/playwright/rebase.ts` so the
// generic Playwright-runtime concern lives in annot-playwright
// instead of the Astro integration package.
//
// `locator.screenshot()` returns a PNG cropped to the locator's
// bounding box. Annotation coordinates (whether resolver-derived
// from an MDX snapshot or caller-supplied via `annot.overlays`)
// live in PAGE space. To overlay correctly on the cropped image,
// every coordinate needs to be rebased by `(clip.x, clip.y)`, and
// overlays whose bbox falls fully outside the clip must be dropped
// (warning + skip per Open Question 4 of the parent plan; no
// fail-fast in v1).
//
// All functions are pure — `clip` in, annotations in, rebased
// annotations + dropped diagnostic out. The patch's `runAnnotMode`
// calls this once per locator screenshot.
 ...
 * Partial overlap drops the annotation — see Open Question 4 of
 * the parent plan (warning + skip, not fail-fast; future
 * `annot.strictClip` could opt into stricter containment if
 * needed).
 */
