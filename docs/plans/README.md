...
| [`tool-property-renderer-schema.md`](./tool-property-renderer-schema.md) | Queued | Final piece of the schema-driven trilogy after `_done/property-panel-schema.md` (#153–#161) and `_done/toolbar-schema.md` (#166–#171). Five phases declarativising the Tool-side property panel ([`packages/web/src/editor/tool-property-renderer.ts`](../../packages/web/src/editor/tool-property-renderer.ts), 649 LOC, 14 `if (isXxx)` branches): Tier B `panelControls` per `TOOL_REGISTRY` entry + Tool-side `ToolPanelAdapter` that delegates label / options / range metadata into the SELECTION registry, eliminating drift between the two surfaces. Target: drop the renderer to ~150 LOC. |
| [`toolbar-apply-style-to-element.md`](./toolbar-apply-style-to-element.md) | Queued | Follow-up to `_done/toolbar-schema.md` Phase 5 ([#170](https://github.com/ingcreators/annot/pull/170)). Adds `applyStyleToElement?: (el, preset) => void` to `ToolRegistryEntry` as the inverse of the existing `extractStyleFromElement`, then collapses `applyPresetStyleAttrs` / `applyMarkerPresetStyle` / `applyTextboxPresetStyle` ([`toolbar-preset-helpers.ts`](../../packages/web/src/editor/toolbar-preset-helpers.ts), ~120 LOC of element-tag cascade) into a generic registry dispatch. Closes the read/write asymmetry the toolbar's schema migration left open; structural test enforces every extractor has a paired writer. Four phases. |
| [`toolbar-highlight-flyout-kind.md`](./toolbar-highlight-flyout-kind.md) | Queued | Cleanup follow-up to `_done/toolbar-schema.md`. Lifts the last residual `if (toolId === "highlight")` branch sites (3 callsites in [`toolbar.ts`](../../packages/web/src/editor/toolbar.ts) + [`toolbar-canvas-menu.ts`](../../packages/web/src/editor/toolbar-canvas-menu.ts)) into a `flyoutKind: "variant" \| "color"` discriminator + `chipColorForVariant` / `tooltipLabelForVariant` accessors on `ToolRegistryEntry`. After this lands, adding a future swatch-style tool (e.g. "Stamp") is one registry entry; structural grep-test guards the no-`toolId === "..."`-literal invariant. Five phases. |
| [`office-paste-abi-modernisation.md`](./office-paste-abi-modernisation.md) | Queued | Follow-up to the pre-release schema-lock series ([#180](https://github.com/ingcreators/annot/pull/180)–[#186](https://github.com/ingcreators/annot/pull/186)) — the one item that didn't fit a single-side delete. The Office-paste TS↔Rust ABI uses `text` / `stroke` to carry data the Rust struct doesn't model (marker shape, mosaic PNG, sticky bg color, callout tail, redact-style flag). Eight phases, each gated by a GVML-XML golden snapshot: Phase 0 lands the snapshot, Phases 1–7 migrate one carrier per phase to a properly-named Rust field, Phase 8 cleans up + archives. After this lands, every TS field has a matching Rust field; no comments saying "Rust side reads from" / "carrier field". |
