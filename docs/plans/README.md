...
| Plan | Status | Summary |
|------|--------|---------|
| [`vendor-libimagequant.md`](./vendor-libimagequant.md) | Draft | Replace [`@panda-ai/imagequant`](https://www.npmjs.com/package/@panda-ai/imagequant) (2-month-old npm org with one follower, 0 GitHub stars on the upstream repo, no CI on the WASM build, 331 weekly downloads) with an in-tree wasm-bindgen wrapper around the upstream [`ImageOptim/libimagequant`](https://github.com/ImageOptim/libimagequant) Rust crate. Same algorithm (Median Cut + Voronoi), same runtime ABI as consumed by [`packages/core/src/encode/index.ts:19,127,152`](../../packages/core/src/encode/index.ts:19) — the swap is one import-path change, gated by a byte-equivalence golden so the migration is provably algorithm-equivalent. Five phases: (1) new `packages/imagequant/` workspace package + committed `pkg/` artifact; (2) CI verify-wasm job; (3) byte-equivalence golden test; (4) import swap + remove `@panda-ai/imagequant` dep; (5) docs cleanup. WASM is committed (no Rust required for `pnpm install`); `pnpm build:wasm` reproduces it from source for audit. Considered alternatives (UPNG.js, jSquash) ruled out in plan body — UPNG has no standalone quantizer API, jSquash has no quantizer at all. **Not** changing the algorithm. **Not** publishing the new package to npm. |
| [`lit-migration-completion.md`](./lit-migration-completion.md) | Draft | Finishes the original Lit migration ([`_done/lit-migration.md`](./_done/lit-migration.md)) by covering its deferred Phase 5c (the `toolbar.ts` orchestrator at 1,604 LOC) **and** the five vanilla DOM components surfaced by the 2026-04-27 friction audit ([`tag-editor.ts`](../../packages/web/src/editor/tag-editor.ts), [`scratchpad-section.ts`](../../packages/web/src/editor/scratchpad-section.ts), [`split-editor.ts`](../../packages/web/src/editor/split-editor.ts), [`gallery-page.ts`](../../packages/web/src/gallery/gallery-page.ts), [`context-menu.ts`](../../packages/web/src/gallery/context-menu.ts)). Six phases ordered smallest → largest so the recipe (light DOM, `annot-` prefix, runtime `static properties` API) is established by the easy phases before the hardest ones land. Phase 6 (toolbar) further splits into 6a flyouts / 6b save menu / 6c residual orchestrator. Each phase ships one Storybook story, lifting the 5/22 ratio acknowledged by [#236](https://github.com/ingcreators/annot/pull/236) to ~11/27 by completion. Tier C live-editor primitives in [`packages/editor/src/`](../../packages/editor/src/) stay imperative — explicitly out of scope. |
