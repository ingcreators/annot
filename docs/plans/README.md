...
| [`pptx-export-shared-builder-finish.md`](./pptx-export-shared-builder-finish.md) | Queued | Direct follow-up to `_done/office-paste-shared-drawing-builder.md`. The parent plan migrated the simple shapes (rect / ellipse / text / marker / freehand-single / mosaic) to the shared `@ingcreators/annot-render` builder and left ~10 helpers + 3 per-shape emitters in `pptx-export.ts` because they either duplicate render/drawingml/helpers verbatim or block on shape-model gaps (curved arrows, freehand-group wrapper). This plan finishes the migration on **ArrowTool's current output surface only** — legacy `<line>` saved-data salvage is explicitly out of scope and dropped as dead code. Five phases: phase 0 pins the curved-arrow path in the PPTX golden, phase 1 drops the drop-in helper duplicates (`colorHex` / `endOOXML` / `capOOXML` / `joinOOXML` / `parseSVGPath` / EMU constants) and the legacy `tag === "line"` dispatch arm, phase 2 routes `buildFreehandGroup`'s child emit through the shared builder (`buildFreehand` disappears), phase 3 widens `AnnotationShape` with `arrow_curve_cx` / `arrow_curve_cy` so `buildLine` (and `paintXml` / `strokeOpacity` / `_lnXml` / SVG-side `xfrmAttrs` / pptx-side `gradFillXml`) all disappear, phase 4 cleanup + archive. End state: `pptx-export.ts` ≤500 LOC (slide envelope + packaging only); the CLAUDE.md guardrail "new tool = one `transformOf` mapping + one per-shape builder" becomes literally true. |
