...
| [`office-paste-shared-drawing-builder.md`](./office-paste-shared-drawing-builder.md) | Queued | Direct follow-up to `_done/office-paste-abi-modernisation.md`. Collapse the parallel OOXML builders (`packages/editor/src/pptx-export.ts` ~1k LOC + `packages/desktop/src-tauri/src/commands/clipboard.rs` ~600 LOC) into a single shared TS implementation under `@ingcreators/annot-render`. Six phases: phase 0 lands a PPTX export golden snapshot, phase 1 pipes `pptx-export` through `transformOf` so both surfaces share the `AnnotationShape[]` input, phase 2 introduces `buildShapeXml` in `annot-render` (parameterised on `a:` vs `p:` namespace), phase 3 collapses the Rust crate to packaging-only (Rust loses ~600 LOC), phase 4 closes the parity gaps both surfaces gained for free (callout tail in PPTX, no-outline solid redaction), phase 5 cleanup + archive. After landing, a new tool means one `transformOf` mapping + one per-shape builder; PPTX export and Office paste pick it up automatically. |
