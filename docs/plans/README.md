...
| [`plugin-storage-registration.md`](./plugin-storage-registration.md) | Draft | Open `storage/bridge.ts` to plugin-registered backends so `annot-cloud`'s pointer-commit store can land without a fork. First of three Phase-5-deferred follow-ups. |
