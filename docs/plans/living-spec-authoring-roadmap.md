...
| Sub | Output | Scope |
|---|---|---|
| **4a** | `StorageWithDocuments` capability extension — adds optional `getAnnotationsYaml(pngPath) → string \| undefined` and `setAnnotationsYaml(pngPath, content)` to the interface in `@ingcreators/annot-core/storage/types`, plus a dedicated `supportsAnnotationsYaml` predicate. Per-store implementations land in the same PR for the four storage backends that ship a full `StorageWithDocuments` surface today: `DeviceStore` (File System Access API), `DesktopStore` (Node `fs/promises` via Electron IPC), `GitHubStore` (Contents + Git Data API), `GoogleDriveStore` (Drive API v3). (VSCode's webview uses a per-file `MessageProxyStorageProvider` that doesn't implement `StorageWithDocuments`; the yaml capability lands on a proper `VSCodeStore` when Phase 6's `.annot.mdx` custom editor introduces it — see [Phase 6 sub-phase 6d](#sub-phases-1).) Each store resolves the yaml path by convention `<pngPath>.annotations.yaml` (`shots/login.png` → `shots/login.png.annotations.yaml`) so callers pass just the PNG path; per-store storage primitives handle the actual file IO. Vitest coverage per implementation; a capability-predicate test in the core types unit test. | New Tier A surface (capability interface + predicate) + per-store impls as thin wrappers around existing read/write paths. Independent of editor changes — usable by any future caller (CLIs, MCP tools, future Playwright fixtures). |
...
| **4f** | EditorShell + OverlayTool integration dogfood + opportunistic "Save as flat PNG" affordance using `flattenEditablePng`. host-ui gains a vitest happy-dom integration test (`editor-shell.overlay.test.ts`) that drives the full 4a→4e surface — `mountFromRecord({ annotationsYamlPath })` → load yaml via Phase 4b loader → `createOverlayToolContext` → `OverlayTool.setContext` + `handlePick` → `openIntentDialog` confirm → `onCommit` → `publishOverlay` → Phase 4b writer → in-memory `StorageProvider` stub. A workflow-app Playwright dogfood that activates the same flow inside a real editor app is reserved for a follow-up once the editor's deployed surface includes a publish-flat affordance + an intent dialog UI (a piece neither 4a–4e nor 4f ship). Additionally, `<annot-save-menu>` gets a new "Save as flat PNG" entry backed by `flattenEditablePng` (re-exported from `@ingcreators/annot-annotator/flatten` — narrow subpath so host-ui's browser build doesn't pull in annotator's Node-only canvas / resvg deps). The `EditorShell.publishFlatPng()` helper renders the current canvas via `getPngDataUrl` + threads the bytes through `flattenEditablePng` to strip any recoverable editable-layer chunks. The menu entry only renders when the host wires `publishFlatPng` into the menu's `SaveMenuContext` — non-editor save menus stay unaffected. (Diff-preview via `diffScreenshots` and region-burn via `burnRegions` are reserved for follow-ups — Phase 4 keeps the scope tight.) | Vitest integration dogfood + the first user-visible integration of the Phase 3 follow-up #2 annotator primitives. The integration test is the canonical verification that the entire Phase 4 surface holds together end-to-end (10 cases covering load / publish / context-snapshotting / cross-layer wiring). The `flattenEditablePng` integration is two function calls (host-ui invokes the annotator primitive); no new architecture. |
...
- `packages/host-ui/src/editor-shell.overlay.test.ts` — vitest integration dogfood (4f); a workflow-app Playwright follow-up is reserved for when the editor's deployed surface includes a publish-flat + intent-dialog UI

#### Files modified

- `packages/core/src/storage/types.ts` — `StorageWithDocuments` extension + `supportsAnnotationsYaml` predicate (4a)
